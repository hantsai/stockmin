// StockMin PWA — Real data + Watchlist management + AI analysis
import { useState, useRef, useEffect, useCallback } from "react";
import { LineChart, Line, ResponsiveContainer, YAxis } from "recharts";

// ── Design Tokens ────────────────────────────────────────────────────────────
const C = {
  bg: "#080b12", surface: "#0d1120", card: "#111827", cardHi: "#161f32",
  border: "#1c2540",
...
// ── Fallback Mock Data ───────────────────────────────────────────────────────
const MOCK = {
  "2330.TW": { name:"台積電", price:1045, pct:2.45,  change:25,   low:1018, high:1052, currency:"TWD", sparkline: genMock(1045, true)  },
  "2317.TW": { name:"鴻海",   price:198,  pct:-1.73, change:-3.5, low:195,  high:202,  currency:"TWD", sparkline: genMock(198,  false) },
  "2454.TW": { name:"聯發科", price:1580, pct:2.59,  change:40,   low:1540, high:1595, currency:"TWD", sparkline: genMock(1580, true)  },
  "AAPL":    { name:"Apple",  price:213,  pct:1.48,  change:3.1,  low:209,  high:215,  currency:"USD", sparkline: genMock(213,  true)  },
  "NVDA":    { name:"NVIDIA", price:1087, pct:-1.36, change:-15,  low:1060, high:1105, currency:"USD", sparkline: genMock(1087, false) },
  "TSLA":    { name:"Tesla",  price:249,  pct:3.62,  change:8.7,  low:238,  high:252,  currency:"USD", sparkline: genMock(249,  true)  },
};
function genMock(base, up) {
  let v = base * 0.96;
  return Array.from({ length: 24 }, () => {
    v += (Math.random() - (up ? 0.42 : 0.58)) * base * 0.008;
...
async function fetchQuote(ticker) {
  try {
    const url = encodeURIComponent(
      `https://query1.finance.yahoo.com/v8/finance/chart/${ticker}?interval=5m&range=1d&includePrePost=false`
    );
    ...
    // Build sparkline: use intraday closes, append current price
    const spark = closes.length > 4
      ? [...closes.slice(-23).map(v => ({ v: +v.toFixed(2) })), { v: now }]
      : genMock(now, now >= prev);
    return {
      ticker,
    ...
    // Fallback to mock
    const m = MOCK[ticker];
    if (m) return { ticker, ...m, real: false };
    return null;
  }
}
...
async function loadWatchlist() {
  try {
    const r = await window.storage.get("stockmin:watchlist");
    return r ? JSON.parse(r.value) : DEFAULT_TICKERS;
  } catch { return DEFAULT_TICKERS; }
}
async function saveWatchlist(tickers) {
  try { await window.storage.set("stockmin:watchlist", JSON.stringify(tickers)); } catch {}
}
    ...
    <div style={{ position: "relative", marginBottom: 10 }}>
      {editing && (
        <button onClick={() => onRemove(s.ticker)} style={{
          ...
          background: C.red, color: "#fff", fontSize: 14, lineHeight: "22px",
          cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
          boxShadow: `0 0 8px ${C.red}`,
        }}>−</button>
      )}
      <div onClick={() => !editing && onTap(s)} style={{
        background: C.card, border: `1px solid ${C.border}`, borderRadius: 18,
        padding: "16px 18px", cursor: editing ? "default" : "pointer",
        transition: "transform .15s, border-color .15s",
        borderColor: editing ? C.dim : C.border,
      }}>
        {/* Row 1 */}
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:10 }}>
          ...
          <div style={{ background:bg, border:`1px solid ${bd}`, borderRadius:10, padding:"5px 12px", display:"flex", alignItems:"center", gap:4 }}>
            <span style={{ fontSize:16, lineHeight:1 }}>{up?"▲":"▼"}</span>
...
function RecCard({ r, type }) {
  ...
  const bg    = isBuy ? C.greenBg : C.redBg;
  const bd    = isBuy ? C.greenBd : C.redBd;
  const up    = r.pct >= 0;
  return (
    <div style={{ background:C.card, border:`1px solid ${bd}`, borderRadius:16, padding:"14px 14px", marginBottom:10 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:8 }}>
        ...
        {r.real && <span style={{ fontSize:10, color:C.sub }}>· 即時</span>}
      </div>
...
function AddSheet({ existing, onAdd, onClose }) {
  ...
  const check = async () => {
    ...
    setChecking(true); setStatus("驗證股票代號...");
    const data = await fetchQuote(t);
    setChecking(false);
    if (data) {
      setStatus(`✓ 找到：${data.name}`);
      setTimeout(() => { onAdd(t, data); onClose(); }, 600);
    } else {
      setStatus("✗ 找不到此股票代號");
    }
  };

  const suggestions = ["2330.TW","2454.TW","2308.TW","MSFT","GOOGL","META","AMZN","2382.TW"]
    ...
    <div style={{ position:"fixed", inset:0, zIndex:100, display:"flex", flexDirection:"column", justifyContent:"flex-end" }}>
      ...
      <div style={{ background:C.surface, borderRadius:"24px 24px 0 0", padding:"24px 20px 48px", border:`1px solid ${C.border}`, borderBottom:"none" }}>
        ...
        <div style={{ display:"flex", gap:8, marginBottom:10 }}>
          <input
            value={val} onChange={e => { setVal(e.target.value.toUpperCase()); setStatus(""); }}
      ...
      try {
        ...
        while (true) {
          ...
          for (const line of lines) {
            ...
            try { const obj = JSON.parse(raw); if (obj.delta?.text && !cancelled) setText(p => p + obj.delta.text); } catch {}
          }
        }
      } catch { if (!cancelled) setText("⚠️ 分析暫時無法使用，請稍後再試。"); }
    ...
    <div style={{ position:"fixed", inset:0, zIndex:100, display:"flex", flexDirection:"column", justifyContent:"flex-end" }}>
      ...
      <div style={{ background:C.surface, borderRadius:"24px 24px 0 0", padding:"24px 20px 48px", border:`1px solid ${C.border}`, borderBottom:"none" }}>
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:16, minHeight:100 }}>
          <div style={{ fontSize:11, color:C.sub, marginBottom:8, letterSpacing:1 }}>✦ AI 分析報告</div>
          {loading && !text && (
...
export default function App() {
  ...
  useEffect(() => {
    loadWatchlist().then(saved => { setTickers(saved); });
  }, []);

  // Fetch stock data whenever tickers change
  useEffect(() => {
    if (!tickers.length) return;
    const refresh = async () => {
      setFetching(new Set(tickers));
      const results = await Promise.all(tickers.map(fetchQuote));
      const map = {};
      results.forEach((d, i) => { if (d) map[tickers[i]] = d; else if (MOCK[tickers[i]]) map[tickers[i]] = { ticker: tickers[i], ...MOCK[tickers[i]] }; });
      setStocks(map);
      setFetching(new Set());
  ...
  const generateRecs = async () => {
    ...
    if (!stockList.length) { setRecsLoad(false); return; }
    const prompt = `你是專業股票分析師。根據以下自選股今日表現，給出買入與減碼建議（繁體中文）。
每個建議必須包含：ticker, name, pct, conf（信心分數0-100）, reason（20字以內）。
只回傳 JSON，格式：{"buy":[...],"sell":[...]}

股票資料：
${stockList.map(s => `${s.ticker} ${s.name} 今日${s.pct>0?"+":""}${s.pct}% 現價${s.price}`).join("\n")}`;
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method:"POST", headers:{"Content-Type":"application/json"},
  ...
  const isLoading = fetching.size > 0 && watchData.length === 0;
  ...
  return (
    <div style={{ background:C.bg, minHeight:"100vh", maxWidth:430, margin:"0 auto", fontFamily:C.sans, color:C.text, overflowX:"hidden" }}>
        ...
        <div>
          ...
          {lastFetch && (
            <div style={{ fontSize:10, color:C.sub, marginTop:4 }}>
              更新於 {lastFetch.toLocaleTimeString("zh-TW",{hour:"2-digit",minute:"2-digit"})}
            </div>
          )}
        </div>
      ...
      {/* Content */}
      <div style={{ padding:"0 16px 100px" }} className="fu">
        ...
        {tab==="ai" && (
          <>
            <button onClick={generateRecs} disabled={recsLoad || watchData.length===0} style={{ width:"100%", padding:"14px 0", borderRadius:14, border:"none", background: recsLoad ? C.dim : C.green, color: recsLoad ? C.sub : C.bg, fontWeight:800, fontSize:15, cursor: recsLoad ? "default" : "pointer", marginBottom:20, transition:"all .2s", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
              {recsLoad
                ? <><div style={{ width:14,height:14,border:`2px solid ${C.sub}`,borderTopColor:"transparent",borderRadius:"50%",animation:"spin .8s linear infinite" }} /> 分析中...</>
                : recs ? "↻ 重新分析" : "✦ 開始 AI 分析"}
            </button>
            ...
            {!recs && !recsLoad && (
              <div style={{ background:C.card, border:`1px dashed ${C.border}`, borderRadius:20, padding:40, textAlign:"center" }}>
                ...
                <div style={{ fontSize:14, color:C.sub, lineHeight:1.8 }}>點擊上方按鈕<br />AI 將分析你的自選股<br />給出買入與減碼建議</div>
              </div>
            )}
        ...
        {tab==="watch" && (
          <>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
              ...
              <button onClick={() => setAddOpen(true)} style={{ padding:"6px 14px", borderRadius:99, border:`1px solid ${C.greenBd}`, background:C.greenBg, color:C.green, fontSize:12, fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", gap:4 }}>
                ＋ 新增股票
              </button>
            </div>
            ...
            {watchData.map(s => (
              <WatchCard key={s.ticker} s={s} onTap={setModal} onRemove={removeStock} editing={editing} />
            ))}
          </>
        )}
      </div>
